...
Add to your FastAPI backend on Render.
...
def send_welcome_email(to_email: str, first_name: str) -> bool:
    ...
    <div class="body">
      ...
      <div class="card">
        ...
        <p style="margin:0 0 8px;">
          ...
          <span class="highlight">Add your holdings</span> — enter your tickers, shares, and cost basis. Live prices load automatically.
        </p>
        <p style="margin:0 0 8px;">
          <span class="green">02 &nbsp;</span>
          <span class="highlight">Check your risk score</span> — your AI-powered score updates on every price tick.
        </p>
        <p style="margin:0 0 0;">
          <span class="green">03 &nbsp;</span>
          <span class="highlight">Set a price alert</span> — get notified the moment your target is hit.
        </p>
      </div>
      ...
      <p style="font-size:13px;">You're on the <span class="badge badge-green">FREE PLAN</span> — up to 10 holdings with delayed prices. Upgrade to Pro for real-time data, AI risk scoring, and unlimited holdings.</p>
    ...
    try:
        resend.Emails.send({
            ...
            "to": to_email,
            "reply_to": REPLY_TO,
            "subject": "Welcome to Phantom Traders — your dashboard is ready",
            "html": _base_template(content, f"Your portfolio dashboard is live, {first_name}. Here's how to get started."),
        })
        return True
...
def send_upgrade_email(to_email: str, first_name: str, plan: str, amount: str, billing: str = "monthly") -> bool:
    ...
    features = {
        ...
        "elite": ["Everything in Pro", "Discord Community included", "Multi-portfolio management", "Advanced AI insights", "SMS price alerts", "API access + webhooks"],
    }.get(plan, [])
    ...
    <div class="body">
      ...
      <div class="stat-row">
        <div class="stat">
          <div class="stat-val">{plan_cap}</div>
        ...
        <div class="stat">
          <div class="stat-val">{billing.capitalize()}</div>
          <div class="stat-label">Billing</div>
        </div>
      </div>

      <div class="card">
        <p style="margin:0 0 12px;font-size:13px;font-weight:700;color:#e8ecf0;">
          <span class="badge {badge_class}">{plan_cap.upper()}</span> &nbsp;WHAT'S NOW UNLOCKED
        </p>
      ...
      {'<div class="card"><p style="margin:0 0 8px;font-size:13px;font-weight:700;color:#e8ecf0;">💬 JOIN THE DISCORD</p><p style="margin:0;font-size:13px;">Your Elite plan includes full Discord Community access. Get live signals, pre-market voice calls, and connect with other serious traders.</p><br><a href="https://discord.gg/cU9ywjWh" class="btn-ghost">Join Discord Server →</a></div>' if plan == "elite" else ''}
    ...
    try:
        resend.Emails.send({
            ...
            "to": to_email,
            "reply_to": REPLY_TO,
            "subject": f"You're on {plan_cap} — welcome to the next level",
            "html": _base_template(content, f"Your {plan_cap} plan is active. All features are now unlocked."),
        })
        return True
    except Exception as e:
        print(f"Upgrade email error: {e}")
        return False
...
def send_alert_email(to_email: str, first_name: str, symbol: str, condition: str, target: float, current_price: float) -> bool:
    ...
    move_pct = abs((current_price - target) / target * 100)
    ...
    <div class="body">
      ...
      <div class="stat-row">
        <div class="stat">
          <div class="stat-val" style="color:{color};">${current_price:.2f}</div>
          <div class="stat-label">Current Price</div>
        </div>
        <div class="stat">
          <div class="stat-val">${target:.2f}</div>
          <div class="stat-label">Your Target</div>
        </div>
        <div class="stat">
          <div class="stat-val" style="color:{color};">{arrow} {move_pct:.1f}%</div>
          <div class="stat-label">vs Target</div>
        </div>
      </div>

      <div class="card">
        <p style="margin:0;font-size:14px;color:#e8ecf0;">
          <strong>{symbol}</strong> is trading <strong style="color:{color};">{direction} your target of ${target:.2f}</strong> at <strong>${current_price:.2f}</strong>.
        </p>
      </div>
      ...
      <div class="divider"></div>
      <p style="font-size:13px;">This alert has been marked as triggered. Set a new alert in your <a href="https://aiphantomtraders.com/#demo" style="color:#00e5a0;">Alerts dashboard</a>.</p>
    </div>"""
    ...
    try:
        resend.Emails.send({
            ...
            "to": to_email,
            "reply_to": REPLY_TO,
            "subject": f"🔔 {symbol} alert triggered — now ${current_price:.2f}",
            "html": _base_template(content, f"{symbol} just crossed your ${target:.2f} target. Current price: ${current_price:.2f}."),
        })
        return True
    except Exception as e:
...
def send_weekly_digest(to_email: str, first_name: str, portfolio_value: float, week_change: float, week_change_pct: float, top_mover: dict = None) -> bool:
    ...
    if top_mover:
        ...
        top_mover_html = f"""
        <div class="card">
          <p style="margin:0 0 8px;font-size:11px;font-family:monospace;color:#4a5568;text-transform:uppercase;letter-spacing:.8px;">Top Mover This Week</p>
          <p style="margin:0;font-size:16px;font-weight:700;color:#e8ecf0;">{top_mover.get('symbol')} &nbsp;
            <span style="color:{tm_color};">{sign}{abs(top_mover.get('change', 0)):.1f}%</span>
          </p>
        </div>"""
    ...
    <div class="body">
      ...
      <div class="stat-row">
        <div class="stat">
          <div class="stat-val">${portfolio_value:,.0f}</div>
          <div class="stat-label">Portfolio Value</div>
        </div>
        <div class="stat">
          <div class="stat-val" style="color:{change_color};">{arrow} {sign}${abs(week_change):,.0f}</div>
          <div class="stat-label">7-Day Change</div>
        </div>
        <div class="stat">
          <div class="stat-val" style="color:{change_color};">{sign}{abs(week_change_pct):.1f}%</div>
    ...
    try:
        resend.Emails.send({
            ...
            "to": to_email,
            "reply_to": REPLY_TO,
            "subject": f"Your weekly portfolio digest — {arrow} {sign}{abs(week_change_pct):.1f}% this week",
            "html": _base_template(content, f"Portfolio: ${portfolio_value:,.0f} · Week: {sign}{abs(week_change_pct):.1f}%"),
        })
        return True
    except Exception as e:

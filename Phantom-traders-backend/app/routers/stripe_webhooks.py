...
async def stripe_webhook(request: Request):
    ...
    # ── customer.subscription.created / updated ────────────────
        ...
        # Extract price ID from subscription items — handle both SDK objects and dicts
        plan = "pro"
        try:
            items_data = []
            # Try as Stripe object first
                ...
                try:
                    ...
                    # Try as dict
                        ...
                        if isinstance(price, dict):
                            plan = price_to_plan(price.get("id", ""))
                        elif hasattr(price, 'id'):
                            plan = price_to_plan(price.id)
                except Exception as e:
                    logger.warning(f"Error extracting price from item: {e}")
                break  # use first item only
        except Exception as e:
            logger.warning(f"Error processing subscription items: {e}")

        # Safely get period end from subscription (not items)
        period_end = None
        try:
            raw_end = None
            ...
            # If not on subscription, try first item
            ...
            logger.warning(f"Error getting period_end: {e}")

        # Find user — try metadata first, then customer lookup
        user_id = find_user_id_from_metadata(data)
        if not user_id:
            user_id = find_user_by_customer(sb, customer_id)

        logger.info(f"Subscription {sub_status}: customer={customer_id} user={user_id} plan={plan}")

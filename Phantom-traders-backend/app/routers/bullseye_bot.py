...
CH_SQUEEZE_ALERTS  = "squeeze-alerts"
ROLE_ELITE = "Elite"
ROLE_MOD   = "Moderator"
...
alerted:        set[str]        = set()
squeeze_alerted: set[str]       = set()
last_scan_dt:   datetime | None = None
scan_count:     int             = 0
total_alerts:   int             = 0
total_squeezes: int             = 0
...
# ── SQUEEZE SCORING ENGINE ────────────────────────────────────────────────────

def score_squeeze(ticker: str, quote: dict, ind: dict) -> dict | None:
    """
    Detect short squeeze candidates using:
      1. Bollinger Band Squeeze (bands narrowing = coiled spring)
      2. RSI building momentum (45-65 range trending up)
      3. Volume expansion (surge above average)
      4. MACD histogram turning positive
      5. Price holding above BB middle (bullish bias)
      6. ATR expanding (volatility returning after squeeze)
    """
    price     = quote.get("c", 0)
    prev      = quote.get("pc", 1)
    high      = quote.get("h", price)
    ...
    if not bb_middle or not price: return None

    bb_width      = (bb_upper - bb_lower) / bb_middle
    atr_ratio     = atr / atr_prev if atr_prev else 1
    rsi_momentum  = rsi - rsi_prev
    intraday_range = (high - low) / prev * 100 if prev else 0
    ...
    # ── Factor 1: BB Squeeze (most important) ─────────────────────────────────
    if bb_width < 0.03:
        score += 35; signals.append(f"🔥 EXTREME BB Squeeze (width: {bb_width:.4f}) — maximum compression")
    elif bb_width < 0.05:
        score += 25; signals.append(f"🔥 Strong BB Squeeze (width: {bb_width:.4f}) — coiled spring setup")
    elif bb_width < 0.07:
        score += 15; signals.append(f"BB Squeeze forming (width: {bb_width:.4f}) — volatility contracting")
    else:
        return None  # No squeeze — skip entirely

    # ── Factor 2: RSI momentum building ───────────────────────────────────────
    if 50 <= rsi <= 70 and rsi_momentum > 0:
        score += 20; signals.append(f"RSI building momentum ({rsi:.0f} ↑{rsi_momentum:+.1f}) — bullish pressure")
    elif 45 <= rsi <= 65:
        score += 12; signals.append(f"RSI neutral-bullish ({rsi:.0f}) — healthy for squeeze")
    elif rsi < 35:
        score += 8; signals.append(f"RSI oversold ({rsi:.0f}) — potential spring loaded bounce")

    # ── Factor 3: MACD turning positive ───────────────────────────────────────
    if hist_prev <= 0 < macd_hist:
        score += 20; signals.append("MACD histogram turning positive 🔄 — momentum shift confirmed")
    elif macd_hist > 0 and macd_hist > hist_prev:
        score += 15; signals.append("MACD histogram expanding positive — momentum accelerating")
    elif macd > macd_sig:
        score += 8; signals.append("MACD bullish crossover — directional bias up")

    # ── Factor 4: Price above BB middle (bullish bias) ────────────────────────
    if price > bb_middle * 1.005:
        score += 10; signals.append(f"Price holding above BB midline (${bb_middle:.2f}) — bullish bias")
    elif price > bb_middle * 0.995:
        score += 5; signals.append(f"Price at BB midline — neutral, watching for breakout direction")

    # ── Factor 5: ATR expanding (volatility returning) ────────────────────────
    if atr_ratio >= 1.3:
        score += 15; signals.append(f"Volatility EXPANDING — ATR {atr_ratio:.2f}x — squeeze breaking out!")
    elif atr_ratio >= 1.1:
        score += 8; signals.append(f"Volatility starting to expand — ATR {atr_ratio:.2f}x")

    # ── Factor 6: Price momentum on the day ───────────────────────────────────
    if chg_pct >= 2.0:
        score += 10; signals.append(f"Strong upside move +{chg_pct:.1f}% — squeeze may be firing")
    elif chg_pct >= 0.5:
        score += 5; signals.append(f"Positive price action +{chg_pct:.1f}% — building momentum")
    elif chg_pct <= -2.0:
        score -= 10  # negative penalty — bearish day during squeeze is bad signal

    # Confluence bonus
    if len(signals) >= 4:
        score += 10; signals.append(f"⚡ {len(signals)} squeeze factors aligned — high conviction")

    # Normalize to 0-100
    score = min(100, int(score / 120 * 100))

    if score < MIN_CONFIDENCE or len(signals) < 2:
        return None

    # Suggested entry
    step   = 1 if price < 20 else (2 if price < 50 else (5 if price < 200 else 10))
    atm    = round(price / step) * step
    strike = atm + step  # slightly OTM call for squeeze plays

    now = datetime.now(ET)
    days_to_fri = (4 - now.weekday()) % 7 or 7
    expiry_weekly  = (now + timedelta(days=days_to_fri)).strftime("%m/%d")
    expiry_monthly = (now + timedelta(days=21)).strftime("%m/%d")

    conf_label_map = {90: "🔥 EXTREME", 80: "🟢 HIGH", 70: "🟡 SOLID"}
    conf_label = "🟠 MODERATE"
    for threshold, label in conf_label_map.items():
        if score >= threshold:
            conf_label = label; break
...
def build_squeeze_embed(signal: dict, scan_num: int) -> discord.Embed:
    conf  = signal["score"]
    bar   = confidence_bar(conf)
    embed = discord.Embed(
        title=f"🌀 SQUEEZE ALERT #{scan_num} — {signal['ticker']} CALL",
        description=(
            f"🌀 **{signal['ticker']}** — Short squeeze setup detected\n"
            f"{bar} **{conf}%** {signal['conf_label']}"
        ),
    ...
    embed.add_field(
        ...
        value=f"**${signal['strike']:.0f} CALL**\nWeekly: {signal['expiry_weekly']} | Monthly: {signal['expiry_monthly']}",
        inline=True
    )
    embed.add_field(
        name="📊 Squeeze Metrics",
        value=(
            f"BB Width: **{signal['bb_width']:.4f}** (lower = tighter squeeze)\n"
            f"ATR Expansion: **{signal['atr_ratio']:.2f}x**\n"
            f"RSI: **{signal['rsi']:.0f}** ({signal['rsi_momentum']:+.1f} momentum)"
        ),
        inline=False
    )
    embed.add_field(
        name=f"⚡ Squeeze Signals ({len(signal['signals'])} factors)",
        value="\n".join(f"→ {s}" for s in signal["signals"]),
        inline=False
    )
    embed.add_field(
        name="📋 How to Play a Squeeze",
        value=(
            "• Enter **at or near the BB midline** for best risk/reward\n"
            "• Use **weekly calls** for aggressive plays\n"
            "• Use **monthly calls** for more time if conviction is high\n"
            "• Stop loss: **close below BB lower band**\n"
            "• Target: **BB upper band** as initial profit zone"
        ),
        inline=False
    )
    embed.add_field(
        name="⚠️ Risk",
        value="Squeezes can fire in **either direction**. Always confirm with price action before entry.",
        inline=False
    )
    embed.set_footer(text=f"Phantom Bullseye • Squeeze Scan #{scan_num} • {NFA_NOTE}")
    return embed
...
async def bullseye_scan():
    ...
    async with aiohttp.ClientSession() as session:
        for ticker in WATCHLIST:
            try:
                ...
                squeeze = score_squeeze(ticker, quote, ind)
                if squeeze: squeezes.append(squeeze)
            except Exception as e:
    ...
    # ── Squeeze alerts ────────────────────────────────────────────────────────
    ch_squeeze = get_ch(guild, CH_SQUEEZE_ALERTS)
    if squeezes:
        squeezes.sort(key=lambda x: x["score"], reverse=True)
        print(f"  [Bullseye #{scan_count}] Found {len(squeezes)} squeeze(s)")
        for sq in squeezes:
            sq_key = f"squeeze_{sq['ticker']}_{now.date()}_{now.hour}"
            if sq_key in squeeze_alerted: continue
            squeeze_alerted.add(sq_key)
            total_squeezes += 1
            if ch_squeeze:
                ping = "@here — 🌀 Squeeze firing!" if sq["score"] >= 85 else ""
                try: await ch_squeeze.send(content=ping or None, embed=build_squeeze_embed(sq, scan_count))
                except discord.Forbidden: pass
            await asyncio.sleep(1)
    else:
        print(f"  [Bullseye #{scan_count}] No squeezes detected")
...
async def status(ctx):
    ...
    await ctx.reply(
        ...
        f"Total scans: {scan_count} | Bullseye alerts: {total_alerts} | Squeeze alerts: {total_squeezes}\n"
...
async def setup_channels(ctx):
    ...
    if ch_squeeze:
        e = discord.Embed(
            title="🌀 Welcome to Squeeze Alerts — Elite Access",
            description=(
                "This channel posts **short squeeze setups** detected by the Bullseye scanner.\n\n"
                "A squeeze occurs when Bollinger Bands compress tightly — like a coiled spring — "
                "before a big explosive move.\n\n"
                "Each alert includes:\n"
                "• BB width (tightness of squeeze)\n"
                "• RSI momentum direction\n"
                "• ATR expansion (volatility returning)\n"
                "• Suggested strike and expiry\n"
                "• How to play the squeeze\n\n"
                "⚠️ Squeezes can fire in **either direction**. "
                "Always confirm with price action before entering."
            ),
            color=discord.Color.from_str("#9B59B6")
        )
        e.set_footer(text=NFA_NOTE)
        try: await ch_squeeze.send(embed=e)
        except discord.Forbidden: pass
    await ctx.reply("✅ Bullseye + Squeeze channels set up!")
...
async def on_ready():
    ...
    print(f"  Channels: #{CH_BULLSEYE_DROPS} | #{CH_BULLSEYE_ALERTS} | #{CH_SQUEEZE_ALERTS}")
    bullseye_scan.start()

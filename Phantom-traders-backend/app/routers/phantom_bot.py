...
Deploy on Render as a Background Worker:
  ...
  Start command:  python phantom_bot.py
"""
...
# ── SERVER STRUCTURE ──────────────────────────────────────────────────────────
# (category_name, [(channel_name, topic, access, is_announcement, is_readonly)])
# access: "all" | "pro" | "elite"
...
# Voice channels per category name → list of (name, access)
VOICE_CHANNELS = {
    "🎙 VOICE ROOMS": [
        ("Pre-Market",        "all"),
        ("Options Room",      "pro"),
...
def build_overwrites(guild, access, role_objects, is_readonly=False):
    """Build permission overwrites based on access level."""
    ow = {}
...
async def set_member_tier(member: discord.Member, new_tier: str, reason: str = "Subscription update") -> str:
    """
    Assign a tier role to a member, removing any conflicting tier roles.
    new_tier must be one of: "Rookie", "Pro Trader", "Elite", or "" (remove all).
...
async def promote(ctx, member: discord.Member, tier: str):
    """
    Manually promote a member. Usage: !promote @user Pro Trader
    Tier options: Rookie | Pro Trader | Elite
    """
    tier = tier.title()
...
async def demote(ctx, member: discord.Member, tier: str):
    """
    Manually demote a member. Usage: !demote @user Rookie
    """
    tier = tier.title()
    if tier not in TIER_HIERARCHY:
        await ctx.reply(f"❌ Unknown tier `{tier}`. Use: Rookie, Pro Trader, Elite")
...
async def remove_roles_cmd(ctx, member: discord.Member):
    """Remove all tier roles from a member. Usage: !removeroles @user"""
...
async def whois(ctx, member: discord.Member):
    """Check a member's current tier. Usage: !whois @user"""
...
# If you want to auto-assign roles from Stripe webhooks, call set_member_tier()
# from your FastAPI backend via the Discord REST API, OR expose a simple HTTP
# endpoint here. Example stub below — wire to your Stripe webhook handler.

# async def handle_stripe_event(discord_user_id: int, event_type: str):
#     """
#     Call this from your FastAPI app when a Stripe subscription event fires.
#     event_type: "pro.created" | "pro.cancelled" | "elite.created" | "elite.cancelled"
#     """
...
#         return
#     tier_map = {
#         "pro.created":      ROLE_PRO,
#         "pro.cancelled":    ROLE_ROOKIE,
#         "elite.created":    ROLE_ELITE,
#         "elite.cancelled":  ROLE_PRO,
#     }
#     new_tier = tier_map.get(event_type, ROLE_ROOKIE)
#     await set_member_tier(member, new_tier, reason=f"Stripe: {event_type}")
...
async def setup_server(ctx):
    """
    Scaffold all categories, channels, roles, and permissions.
    Run once. Safe to re-run — skips anything that already exists.
    Usage: !setup_server
    """
    guild = ctx.guild
    ...
    # Step 2: create text categories and channels
    ...
    # Step 3: create voice category + channels
...
def score_setup(ticker: str, quote: dict, indicators: dict) -> dict | None:
    """
    Score a ticker 0–100 for options setup quality.
    Returns setup dict if score >= 45, else None.
    """
    price    = quote.get("c", 0)
    prev     = quote.get("pc", 1)
...
async def fetch_ticker_data(session: aiohttp.ClientSession, ticker: str) -> dict | None:
    ...
    try:
        ...
        indicators = {
            ...
            "avg_volume":  quote.get("v", 0),  # rough — improve with paid Finnhub tier
        }
...
async def scanner_loop():
    ...
    now = datetime.now(ET)
    # Reset alert cache each new day
    ...
    async with aiohttp.ClientSession() as session:
        for ticker in WATCHLIST:
            ...
            if data:
                ...
                if setup:
                    setups.append(setup)
            await asyncio.sleep(0.35)  # ~3 req/sec — respects Finnhub rate limit
    ...
    # Summary to screener-drops (visible to all)
...
async def manual_scan(ctx):
    """Trigger a manual scan now. !scan"""
...
async def show_watchlist(ctx):
    """Show the scanner watchlist. !watchlist"""
...
async def add_watch(ctx, ticker: str):
    """Add a ticker to the watchlist. !addwatch TSLA"""
...
async def remove_watch(ctx, ticker: str):
    """Remove a ticker from the watchlist. !removewatch TSLA"""
    t = ticker.upper().strip()
    if t in WATCHLIST:
        WATCHLIST.remove(t)
...
async def pt_help(ctx):
    ...
    embed.add_field(name="📊 Scanner", value=(
        ...
        "`!addwatch TSLA` — add ticker *(Mod)*\n"
        "`!removewatch TSLA` — remove ticker *(Mod)*"
    ), inline=False)
    embed.add_field(name="👤 Roles *(Mod only)*", value=(
        "`!promote @user Pro Trader`\n"
        "`!demote @user Rookie`\n"
        "`!removeroles @user`\n"
        "`!whois @user`"
    ), inline=False)
    embed.add_field(name="🔧 Server *(Mod only)*", value=(

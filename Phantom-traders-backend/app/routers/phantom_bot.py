"""
Phantom Traders — Discord Bot v3.2
====================================
New in v3.2:
  - 72-hour Pro Trader trial for every new member (weekday hours only)
  - Trial pauses Friday 4pm ET → Monday 9:30am ET
  - DM on join showing trial timer
  - DM warning at 24hrs remaining
  - DM on expiry with upgrade CTA
  - #upgrade-now channel with full upsell post
  - Pro Trader / Elite can read message history in all their channels
"""
...
TRIAL_WEEKDAY_HOURS = 72  # hours of actual market-open time
...
# ── TRIAL TRACKER ─────────────────────────────────────────────────────────────
# {user_id: {"start": datetime, "weekday_seconds_used": float, "warned_24h": bool}}
...
def is_weekday_market_time(dt: datetime) -> bool:
    """Returns True if dt is a weekday (Mon-Fri), regardless of market hours.
    Trial counts all weekday hours — not just market hours."""
    return dt.weekday() < 5
...
def calc_weekday_seconds_elapsed(since: datetime) -> float:
    """Calculate total weekday seconds elapsed since a given datetime.
    ...
    while cur < now:
        ...
        if cur.weekday() < 5:  # Mon-Fri
            total += (next_tick - cur).total_seconds()
        cur = next_tick
    return total

def format_trial_remaining(seconds_used: float) -> str:
    total_secs  = TRIAL_WEEKDAY_HOURS * 3600
    remaining   = max(0, total_secs - seconds_used)
...
def build_overwrites(guild, access, role_objects, is_readonly=False):
    """Build permission overwrites. Pro/Elite always get read_message_history=True."""
    ow   = {}
        ...
        for rname in {"pro": [ROLE_PRO, ROLE_ELITE, ROLE_MOD], "elite": [ROLE_ELITE, ROLE_MOD]}.get(access, []):
            if rname in role_objects:
                ow[role_objects[rname]] = discord.PermissionOverwrite(
                    ...
                    send_messages=send,
                    add_reactions=True,
                    read_message_history=True,  # ← paid users can always scroll back
                )
...
async def start_trial(member: discord.Member):
    """Assign Pro Trader trial role and record start time."""
    guild   = member.guild
    ...
    active_trials[member.id] = {
        ...
        "weekday_seconds_used": 0.0,
        "warned_24h":          False,
        "guild_id":            guild.id,
    }

    # Assign Pro Trader role (keep Rookie)
    try:
        await member.add_roles(pro_role, reason="72-hour Pro Trader trial")
    ...
    try:
        embed = discord.Embed(
            ...
            description=(
                f"Welcome **{member.display_name}**! You've been automatically upgraded to "
                f"**Pro Trader** for **72 weekday hours** so you can experience everything Phantom Traders has to offer.\n\n"
                f"⏱️ **Trial countdown:** Weekday hours only — your trial pauses on weekends so you don't miss out."
            ),
            color=discord.Color.from_str("#00C896"),
            timestamp=datetime.now(ET)
        )
        embed.add_field(
            name="⚡ What You Can Access During Your Trial",
            value=(
                "• 📊 Market Intel — pre-market briefings, earnings, macro\n"
                "• ⚡ Trade Signals — option plays, momentum, scanner alerts\n"
                "• 🤖 Bot features — price alerts, trade callouts, RSI/volume alerts\n"
                "• 📋 Bot trade log — full callout history and W/L stats"
            ),
            inline=False
        )
        embed.add_field(
            name="🚀 Want to Keep Access After Your Trial?",
            value=(
                f"Subscribe before your trial ends to keep uninterrupted access:\n\n"
                f"**[→ Upgrade at {SITE_URL}]({SITE_URL})**\n\n"
                f"• **Pro Trader** — Full market intel + signals + scanner\n"
        ...
        embed.add_field(
            name="📌 Trial Rules",
            value=(
                "• Trial is 72 weekday hours (pauses Fri 4pm ET → Mon 9:30am ET)\n"
                "• You'll receive a 24-hour warning before your trial expires\n"
                "• After expiry you revert to Rookie (free) access\n"
                "• Upgrading anytime keeps your access permanently"
            ),
            inline=False
        )
...
async def expire_trial(member: discord.Member, guild: discord.Guild):
    """Remove Pro Trader trial and downgrade to Rookie."""
    pro_role = discord.utils.get(guild.roles, name=ROLE_PRO)
    if pro_role and pro_role in member.roles:
        try:
            await member.remove_roles(pro_role, reason="72-hour trial expired")
        except discord.Forbidden:
            pass

    active_trials.pop(member.id, None)

    # Send expiry DM with upgrade CTA
    try:
        embed = discord.Embed(
            title="⏰ Your Pro Trader Trial Has Ended",
            description=(
                f"Hey **{member.display_name}** — your 72-hour Pro Trader trial has expired "
                f"and your account has been moved back to **Rookie** access.\n\n"
                f"We hope you got a taste of what Phantom Traders Pro can do! 🚀"
            ),
            color=discord.Color.from_str("#F5C842"),
            timestamp=datetime.now(ET)
        )
        embed.add_field(
            name="🔒 What You've Lost Access To",
            value=(
                "• 📊 Market Intel (pre-market, earnings, macro, sector)\n"
                "• ⚡ Trade Signals (option plays, alerts, scanner)\n"
                "• 🤖 Price alerts, trade callouts, screener drops\n"
                "• 👑 Elite Lounge (if you had Elite)"
            ),
            inline=False
        )
        embed.add_field(
            name="🚀 Keep the Edge — Upgrade Now",
            value=(
                f"Don't lose your momentum. Upgrade to keep full access:\n\n"
                f"**[→ Subscribe at {SITE_URL}]({SITE_URL})**\n\n"
                f"**Pro Trader** — ${20}/mo or ${199}/yr\n"
                f"**Elite** — ${50}/mo or ${499}/yr\n\n"
                f"Link your Discord after subscribing and your role is assigned automatically!"
            ),
            inline=False
        )
        embed.set_footer(text=DISCLAIMER_FULL)
    ...
    if ch:
        try:
            ping_embed = discord.Embed(
                ...
                description=(
                    f"{member.mention} — your 72-hour Pro Trader trial has ended.\n\n"
                    f"Upgrade now to keep full access to trade signals, alerts, and the scanner.\n\n"
                    f"**[→ {SITE_URL}]({SITE_URL})**"
                ),
                color=discord.Color.from_str("#FF4D6A"),
                timestamp=datetime.now(ET)
            )
            ping_embed.set_footer(text=NFA_NOTE)
            await ch.send(embed=ping_embed, delete_after=86400)  # auto-delete after 24hrs
        except discord.Forbidden:
            pass
...
async def warn_trial_24h(member: discord.Member):
    ...
    try:
        embed = discord.Embed(
            ...
            description=(
                f"Hey **{member.display_name}** — just a heads up, your Pro Trader trial "
                f"expires in **24 weekday hours**.\n\n"
                f"Don't lose access to trade signals, market intel, and the scanner!"
            ),
            color=discord.Color.from_str("#F5C842"),
            timestamp=datetime.now(ET)
        )
        embed.add_field(
            name="🚀 Upgrade Before It Expires",
            value=(
                f"**[→ Subscribe at {SITE_URL}]({SITE_URL})**\n\n"
                f"• **Pro Trader** — Live signals, scanner, market intel\n"
                f"• **Elite** — Everything + Elite Lounge + priority alerts\n\n"
...
async def trial_checker():
    """Check all active trials every 5 minutes."""
    ...
    warning_secs    = (TRIAL_WEEKDAY_HOURS - 24) * 3600  # warn at 48h used = 24h left

    to_expire = []
    for user_id, trial in list(active_trials.items()):
        member = guild.get_member(user_id)
        if not member:
            active_trials.pop(user_id, None)
            continue

        # Recalculate weekday seconds elapsed from start
    ...
    for member, guild in to_expire:
        await expire_trial(member, guild)
        await asyncio.sleep(0.5)
...
class StrikeView(discord.ui.View):
    ...
    async def add_strike(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.edit_message(
            content=f"✅ **Strike added** by {interaction.user.mention} — {member.mention} now at **{strikes}/3**.",
            view=None
        )
        if strikes >= 3:
            try:
                await member.send(
                    ...
                    f"Contact us at {SITE_URL} if you believe this is an error."
                )
            ...
            try:
                await member.send(
                    ...
                    f"3 strikes = permanent ban. Review **#rules** at {SITE_URL}."
                )
            except discord.Forbidden: pass
...
async def log_offence(guild, member, reason, message, offence_type):
    ...
    embed   = discord.Embed(title=f"🚨 Moderation Alert — {offence_type}",
    ...
    embed.set_footer(text="Use buttons below to add a strike or dismiss.")
...
async def on_message(message: discord.Message):
    ...
    # External Discord invites → delete + log
    ...
    # Non-approved URLs in non-off-topic → delete + redirect (no strike)
    if not deleted:
        ...
        if bad_urls and channel.name != CH_OFF_TOPIC:
            ...
            try:
                await channel.send(
                    ...
                    f"Please share links in **#off-topic**. Approved financial sources are always welcome in trading channels!",
                    delete_after=20
                )
            except discord.Forbidden: pass

    # Repeat spam → delete + log
    if not deleted:
        uid = member.id
...
async def post_welcome(ch: discord.TextChannel):
    embed = discord.Embed(
        ...
        description=(
            ...
            "Experience everything — signals, scanner, market intel — before deciding to upgrade."
        ),
        color=discord.Color.from_str("#00C896"),
    )
    ...
    embed.add_field(name="🚀 Get Started", value=(
        ...
        f"2️⃣ Check your DMs — your 72hr trial has started!\n"
        f"3️⃣ Visit **#education-portal** — 5 pro trading courses\n"
        f"4️⃣ Type `!pthelp` in **#pt-bot-commands**\n"
        f"5️⃣ Upgrade at **[{SITE_URL}]({SITE_URL})** before trial expires"
    ), inline=False)
    embed.add_field(name="🌐 Platform", value=f"[**{SITE_URL}**]({SITE_URL}) — AI signals, courses, and tools.", inline=False)
    embed.set_footer(text=DISCLAIMER_FULL)
    await ch.send(embed=embed)


async def post_upgrade_now(ch: discord.TextChannel):
    embed = discord.Embed(
        title="🚀 Upgrade to Pro Trader or Elite",
        description=(
            "## Don't Lose Your Edge.\n\n"
            "Your 72-hour trial gave you a taste — now make it permanent.\n\n"
            "Phantom Traders gives you the tools, signals, and community that "
            "serious traders use every single day."
        ),
        color=discord.Color.from_str("#F5C842"),
    )
    embed.add_field(
        ...
        value=(
            ...
            "• 🤖 AI options scanner (every 5 min during market hours)\n"
            "• 🔴 RSI extreme alerts\n"
            "• ⚡ Volume spike alerts\n"
            "• 📣 Trade callouts (!call)\n"
            "• 🔔 Custom price alerts (!alert)\n"
            "• 📋 Full message history in all Pro channels\n\n"
            f"**[→ Subscribe at {SITE_URL}]({SITE_URL})**"
        ),
        inline=False
    )
    embed.add_field(
        ...
        value=(
            ...
            "• 👑 Elite Lounge — exclusive signals and deep-dive analysis\n"
            "• 🔄 MACD reversal alerts (highest conviction setups)\n"
            "• @here pings on HIGH-confidence scanner setups\n"
            "• Priority portfolio reviews\n"
            "• 📋 Full message history in all Elite channels\n\n"
            f"**[→ Subscribe at {SITE_URL}]({SITE_URL})**"
        ),
        inline=False
    )
    embed.add_field(
        name="⚡ How to Upgrade",
        value=(
            f"1. Visit **[{SITE_URL}]({SITE_URL})**\n"
            f"2. Choose Pro Trader or Elite\n"
            f"3. Link your Discord account\n"
            f"4. Role is assigned **automatically** — instant access!"
        ),
        inline=False
    )
    embed.add_field(
        name="🆓 Already on Trial?",
        value=(
            "Check your remaining trial time with `!trialstatus` in **#pt-bot-commands**.\n"
            "Upgrade anytime — your trial hours don't count against your paid subscription!"
        ),
        inline=False
    )
    embed.set_footer(text=DISCLAIMER_FULL)
...
async def post_rules(ch: discord.TextChannel):
    embed = discord.Embed(
        ...
        description="Professional trading community standards. Violations are handled by PhantomBot and staff.",
        color=discord.Color.from_str("#FF4D6A"),
    )
    rules = [
        ("1️⃣ Trading Content Only", "Keep all discussion relevant to markets, trading, and finance. Off-topic content belongs in **#off-topic**."),
        ("2️⃣ No External Server Invites", "Posting Discord invite links to other servers is an instant moderation offence. Zero tolerance."),
        ("3️⃣ Approved Links in Trading Channels", "External links only allowed in **#off-topic**. Approved sources (TradingView, Yahoo Finance, Bloomberg, SEC) are welcome everywhere."),
        ("4️⃣ No Spam", "No repeated messages. Keep conversations meaningful and productive."),
        ("5️⃣ Respect Everyone", "No hate speech, personal attacks, or harassment. This is a professional, inclusive environment."),
        ("6️⃣ No Solicitation", "Zero tolerance for 'DM me for signals', referral links, or unauthorized promotions."),
        ("7️⃣ NFA — Not Financial Advice", "All content is for educational purposes only. Always do your own research before any trade."),
        ("8️⃣ NDA — Non-Disclosure", "All signals, strategies, and tools are proprietary. Do not share or redistribute outside this server."),
        ("9️⃣ 3-Strike Policy", "Offences are flagged by the bot and reviewed by mods.\n**Strike 1** → Warning\n**Strike 2** → Final warning\n**Strike 3** → Permanent ban"),
    ]
    for name, value in rules:
        embed.add_field(name=name, value=value, inline=False)
    embed.add_field(name="✅ What We Love", value="Trade ideas, chart analysis, questions, wins AND losses, and genuine community support.", inline=False)
...
async def post_how_to_roles(ch: discord.TextChannel):
    embed = discord.Embed(
        ...
        description="Phantom Traders uses a tier system. Here's how to level up:",
        color=discord.Color.from_str("#F5C842"),
    )
    embed.add_field(name="🟢 Rookie — Free", value=(
        "**Access:** Community channels + education portal\n"
        "**How:** Auto-assigned on join ✅"
    ), inline=False)
    embed.add_field(name="🔵 Pro Trader Trial — Automatic", value=(
        "**Access:** Full Pro Trader features for 72 weekday hours\n"
        "**How:** Auto-assigned to every new member on join ✅\n"
        "**Note:** Trial pauses on weekends — you don't lose time!"
    ), inline=False)
    embed.add_field(name="🔵 Pro Trader — Paid", value=(
        "**Access:** Market Intel + Trade Signals + Scanner + Alerts + message history\n"
        f"**How:** Subscribe at [{SITE_URL}]({SITE_URL}) → link Discord → instant! ✅"
    ), inline=False)
    embed.add_field(name="🟡 Elite — Premium", value=(
        "**Access:** Everything in Pro + Elite Lounge + MACD reversals + @here alerts + full history\n"
        f"**How:** Subscribe Elite at [{SITE_URL}]({SITE_URL}) → link Discord → instant! ✅"
    ), inline=False)
    embed.add_field(name="🔧 Role Issues?", value=(
        f"1. Link Discord at [{SITE_URL}]({SITE_URL})\n"
        "2. `!trialstatus` — check your trial\n"
...
async def post_education_portal(ch: discord.TextChannel):
    header = discord.Embed(
        ...
        description=(
            ...
            "but because they were **never taught the right way.**\n\n"
            "Phantom Traders courses give you the exact frameworks, strategies, and setups "
            "that professional traders use every day.\n\n"
            "📚 All content is for learning and skill-building only. "
    ...
    courses = [
        ...
        {"emoji":"🔮","title":"Futures Trading Mastery","color":"#9B59B6",
         "hook":"No PDT rule. 23hrs/day. Built-in tax advantages. Futures are the pro's playground.",
         "topics":"• Contracts, margin & leverage explained\n• ES, NQ, CL, GC setups & timing\n• 60/40 tax rule advantage\n• Overnight & pre-market strategy\n• Risk management for leveraged instruments"},
    ...
    cta = discord.Embed(
        ...
        description=(
            ...
            "📌 All content is for learning only. Always do your own DD before any trade."
        ),
        color=discord.Color.from_str("#F5C842"),
    )
...
async def fetch_ticker_data(session, ticker):
    ...
    try:
        ...
        return {"ticker": ticker, "quote": quote, "indicators": {
            "rsi": (rsi_raw.get("rsi") or [50])[-1], "macd": (macd_raw.get("macd") or [0])[-1],
            "macd_signal": (macd_raw.get("signal") or [0])[-1],
            "macd_hist": hist[-1] if hist else 0, "macd_hist_prev": hist[-2] if len(hist) >= 2 else 0,
            "bb_upper": (bb_raw.get("upperband") or [0])[-1], "bb_lower": (bb_raw.get("lowerband") or [0])[-1],
            "volume": quote.get("v", 0), "avg_volume": quote.get("v", 0),
        }}
...
async def fetch_earnings(session):
    try:
        ...
        async with session.get(f"https://finnhub.io/api/v1/calendar/earnings?from={today}&to={week}",
                               headers={"X-Finnhub-Token": FINNHUB_KEY}) as r:
            data = await r.json()
...
def score_setup(ticker, quote, indicators):
    ...
    return {"ticker":ticker,"price":price,"chg_pct":chg_pct,"direction":direction,
            "strike":strike,"signals":signals,"score":score,"confidence":conf,"rsi":rsi,"vol_ratio":vol_ratio}
...
async def post_premarket_briefing(guild):
    ...
    gainers=[(t,p,c) for t,p,c in movers if c>0][:5]; losers=[(t,p,c) for t,p,c in movers if c<0][:5]
...
async def post_earnings_calendar(guild):
    ...
    e = discord.Embed(title="📅 Earnings Watch — Next 7 Days", description="Watchlist tickers reporting this week:",
    ...
    for earn in earnings[:10]:
        ...
        e.add_field(name=f"`{sym}`", value=f"📆 {date} — {when_str}{f' | EPS Est: **${est:.2f}**' if est else ''}", inline=False)
    e.set_footer(text="Always check IV before trading earnings. "+NFA_NOTE); await ch.send(embed=e)
...
async def scanner_loop():
    ...
    guild = bot.get_guild(GUILD_ID)
    if not guild: return
    ch_plays=get_ch(guild,CH_OPTION_PLAYS); ch_elite=get_ch(guild,CH_ELITE_SIGNALS)
    ch_screen=get_ch(guild,CH_SCREENER);   ch_alerts=get_ch(guild,CH_PT_ALERTS)
    setups = []
    async with aiohttp.ClientSession() as session:
        for ticker in WATCHLIST:
            ...
            ind=data["indicators"]; hist=ind.get("macd_hist",0); hist_prev=ind.get("macd_hist_prev",0)
            rsi=ind.get("rsi",50); vol_ratio=ind.get("volume",0)/(ind.get("avg_volume",1) or 1)
            price=data["quote"].get("c",0); prev=data["quote"].get("pc",1)
            chg_pct=((price-prev)/prev*100) if prev else 0
            ...
            if macd_key not in alerted_today and ch_elite and (hist_prev<0<hist or hist_prev>0>hist):
                ...
                rev=discord.Embed(title=f"🔄 MACD {'Bullish' if bull else 'Bearish'} Reversal — {ticker}",
            ...
            if rsi_key not in alerted_today and ch_alerts and (rsi<=25 or rsi>=78):
                bull=rsi<=25
                e=discord.Embed(title=f"🔴 RSI Extreme {'Oversold' if bull else 'Overbought'} — {ticker}",
    ...
    if ch_screen and setups:
        lines=[f"{'📈' if s['direction']=='CALL' else '📉'} **{s['ticker']}** — {s['direction']} | {s['score']}/100 | RSI {s['rsi']:.0f}" for s in setups[:5]]
        summary=discord.Embed(title=f"🤖 Scanner Drop — {now.strftime('%b %d %I:%M %p ET')}",
                              description="\n".join(lines),color=discord.Color.from_str("#4D9FFF"),timestamp=now)
...
async def on_member_join(member: discord.Member):
    ...
    # Welcome DM (sent inside start_trial already)
    # Fallback to welcome channel if DM fails (handled in start_trial)

# ── COMMANDS ──────────────────────────────────────────────────────────────────

@bot.command(name="trialstatus")
async def trial_status(ctx):
    """!trialstatus — check your trial time remaining"""
    trial = active_trials.get(ctx.author.id)
    if not trial:
        # Check if they have Pro role
        pro_role = discord.utils.get(ctx.guild.roles, name=ROLE_PRO)
        if pro_role and pro_role in ctx.author.roles:
            await ctx.reply("✅ You're a **paid Pro Trader** — no trial timer applies!")
        else:
            await ctx.reply(f"📭 No active trial found. Visit **[{SITE_URL}]({SITE_URL})** to subscribe.")
        return
    secs_used  = calc_weekday_seconds_elapsed(trial["start"])
    remaining  = format_trial_remaining(secs_used)
    total_secs = TRIAL_WEEKDAY_HOURS * 3600
    pct_used   = min(100, secs_used / total_secs * 100)
    ...
    embed.add_field(name="⏰ Time Remaining", value=f"**{remaining}** of 72 weekday hours", inline=False)
    embed.add_field(name="📊 Progress",       value=f"{bar} ({pct_used:.0f}% used)",        inline=False)
    embed.add_field(name="📅 Trial Started",  value=trial["start"].strftime("%b %d, %Y %I:%M %p ET"), inline=True)
    embed.add_field(name="ℹ️ Weekend Pause",  value="Trial pauses Fri 4pm → Mon 9:30am ET", inline=True)
    embed.add_field(
        name="🚀 Keep Your Access",
        value=f"[**Upgrade at {SITE_URL}**]({SITE_URL}) before your trial expires!",
        inline=False
    )
    embed.set_footer(text=DISCLAIMER_FULL)
...
async def setup_posts(ctx):
    ...
    for ch_name,fn in tasks_map:
        ...
        else: print(f"  #{ch_name} not found")
...
async def setup_server(ctx):
    ...
    await ctx.reply(
        ...
        f"**Pro Trader** → + Market Intel + Signals + Scanner + History\n"
        f"**Elite** → + Elite Lounge + MACD Reversals + History\n"
        f"**72hr Trial** → Auto-started for every new member!"
    )
...
async def promote(ctx, member: discord.Member, *, tier: str):
    ...
    # Cancel trial if promoting to paid
...
async def give_trial(ctx, member: discord.Member):
    """!givetrial @user — manually start a 72hr trial"""
    await start_trial(member)
    await ctx.reply(f"✅ 72-hour Pro Trader trial started for **{member.display_name}**!")

@bot.command(name="endtrial")
@commands.has_permissions(administrator=True)
async def end_trial(ctx, member: discord.Member):
    """!endtrial @user — manually end a trial"""
    if member.id not in active_trials:
        await ctx.reply(f"❌ **{member.display_name}** has no active trial."); return
    await expire_trial(member, ctx.guild)
    await ctx.reply(f"✅ Trial ended for **{member.display_name}** — downgraded to Rookie.")

@bot.command(name="trials")
@commands.has_permissions(administrator=True)
async def list_trials(ctx):
    """!trials — list all active trials"""
...
async def trade_call(ctx, ticker: str, direction: str, strike: str, expiry: str, target: str, stop: str, *, notes: str=""):
    ...
    ch_plays=get_ch(ctx.guild,CH_OPTION_PLAYS); ch_log=get_ch(ctx.guild,CH_TRADE_LOG)
...
async def view_calls(ctx):
    ...
    e=discord.Embed(title=f"📋 Open Callouts ({len(open_calls)})",color=discord.Color.from_str("#4D9FFF"),timestamp=datetime.now(ET))
...
async def close_call(ctx, call_id: int, result: str, exit_price: float=0.0):
    ...
    ch_wins=get_ch(ctx.guild,CH_WINS); ch_plays=get_ch(ctx.guild,CH_OPTION_PLAYS); ch_log=get_ch(ctx.guild,CH_TRADE_LOG)
...
async def set_alert(ctx, ticker: str, price: float):
    ...
    current=quote.get("c",0); direction="above" if price>current else "below"
    price_alerts.append({"user_id":ctx.author.id,"channel_id":ctx.channel.id,"ticker":ticker,"target":price,"direction":direction})
...
async def add_watch(ctx, ticker: str):
    ...
    else: WATCHLIST.append(t); await ctx.reply(f"✅ Added `{t}` — {len(WATCHLIST)} total.")
...
async def pt_help(ctx):
    e=discord.Embed(title="👻 Phantom Traders Bot v3.2 — Commands",color=discord.Color.from_str("#00C896"))
    e.add_field(name="⏱️ Trial *(anyone)*",value="`!trialstatus` — check your trial time remaining",inline=False)
    e.add_field(name="📣 Callouts *(Pro/Elite)*",value=(
    ...
    e.add_field(name="👤 Roles *(Admin)*",value=(
        "`!promote @user Pro Trader` / `!promote @user Elite`\n"
        "`!demote @user Rookie` | `!removeroles @user` | `!whois @user`"),inline=False)
    e.add_field(name="⏱️ Trials *(Admin)*",value=(
        "`!givetrial @user` | `!endtrial @user` | `!trials`"),inline=False)
    e.add_field(name="🔨 Mod *(Admin)*",value="`!strikes @user` | `!clearstrikes @user`",inline=False)
    ...
    e.add_field(name="🏷️ Tiers",value=(
        ...
        "**Elite** → + Elite Lounge + MACD + Priority alerts + History"),inline=False)
...
@bot.event
async def on_ready():
    print(f"\n✓ Phantom Traders Bot v3.2 online — {bot.user}")
